...
Coding Switch 是一款专为 JetBrains IDE 打造的 AI 编程 CLI 与 IDE 辅助工具集。它可以在 IDE 内统一管理 Claude Code、Codex、OpenCode 等主流 AI 编程命令行工具，并提供行内 AI 代码补全、Git 提交信息生成、路径行号插入、CLI 快速启动与版本检测等日常开发能力。

## 核心功能

- **AI CLI 配置管理 (Provider)**：轻松管理和批量切换 Claude Code、Codex (OpenAI) 和 OpenCode 的 API 配置。支持官方接口以及自定义第三方或本地大模型接口，内置多个模型服务商预设（DeepSeek、智谱 GLM、MiniMax、Kimi、百度千帆、阿里通义等），并支持激活前连接测试与健康检查。
