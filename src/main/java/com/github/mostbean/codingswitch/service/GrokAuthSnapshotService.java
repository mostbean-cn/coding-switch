...
public final class GrokAuthSnapshotService {
    ...
    private static final String SERVICE_NAME = "com.github.mostbean.codingswitch.grok.auth.snapshot";
    private static final String SHARED_NAMESPACE = "grok";
    private static final String SNAPSHOT_USERNAME = "grok-official-login";

    public static GrokAuthSnapshotService getInstance() {
        return ApplicationManager.getApplication().getService(GrokAuthSnapshotService.class);
    }
    ...
    private void captureFromLive(String primaryKey, String legacyKey) {
        ...
        String rawAuthJson = configFileService.readGrokAuthRaw();
        if (GrokAuthSupport.isValidOfficialLoginAuth(rawAuthJson)) {
    ...
    private RestoreResult restoreToLive(String primaryKey, String legacyKey) throws IOException {
        ...
        if (snapshotKey != null) {
            ...
            if (GrokAuthSupport.isValidOfficialLoginAuth(rawAuthJson)) {
                ...
                configFileService.writeGrokAuthRaw(rawAuthJson.trim());
                return RestoreResult.RESTORED;
            }
            clearSnapshot(primaryKey, legacyKey);
        }

        String liveAuthJson = configFileService.readGrokAuthRaw();
        if (GrokAuthSupport.isValidOfficialLoginAuth(liveAuthJson)) {
            saveSnapshot(primaryKey, liveAuthJson.trim());
            clearLegacySnapshot(primaryKey, legacyKey);
            return RestoreResult.RESTORED;
        }
        return RestoreResult.NO_SNAPSHOT;
