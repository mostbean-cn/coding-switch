...
public final class I18n {
        ...
        private static Map<String, String> createZhMessages() {
                ...
                m.put("common.button.refresh", "刷新");
                m.put("common.button.clearSelection", "清空勾选");
                ...
                m.put("settings.section.backupSync", "备份同步");
                m.put("settings.section.extension", "拓展功能");
                ...
                m.put("settings.option.syncStatus.unsynced", "未同步");
                m.put("settings.sync.direction.toCcSwitch", "导出到 CC Switch");
                m.put("settings.sync.direction.toCodingSwitch", "导入到 Coding Switch");
                m.put("settings.sync.scope.saved", "已保存配置");
                m.put("settings.sync.status.synced", "已同步");
                m.put("settings.sync.status.unsynced", "未同步");
                m.put("settings.sync.status.loading", "正在加载同步项...");
                ...
                // ── 备份同步（WebDAV）──
                m.put("settings.backup.dialog.title", "WebDAV 备份配置");
                m.put("settings.backup.hint", "通过 WebDAV 协议备份插件配置到云存储（坚果云、Nextcloud 等），实现多设备同步。");
                m.put("settings.backup.label.url", "WebDAV 地址:");
                m.put("settings.backup.label.username", "用户名:");
                m.put("settings.backup.label.password", "密码:");
                m.put("settings.backup.checkbox.rememberPassword", "记住 WebDAV 应用密码");
                m.put("settings.backup.label.remotePath", "远端快照路径:");
                m.put("settings.backup.hint.remotePath", "上传时会自动创建缺失的父目录。");
                m.put("settings.backup.checkbox.autoUpload", "自动上传快照");
                m.put("settings.backup.label.autoUploadInterval", "自动上传间隔:");
                m.put("settings.backup.label.interval", "每");
                m.put("settings.backup.label.intervalUnit", "分钟");
                m.put("settings.backup.hint.autoUpload", "仅在 IDE 打开时运行。下载恢复仍需手动确认，避免意外覆盖本地配置。");
                m.put("settings.backup.hint.plain", "默认只同步连接信息和设置，不会同步 API 密钥等敏感信息。");
                m.put("settings.backup.checkbox.encrypt", "加密备份");
                m.put("settings.backup.hint.encryptPassphrase", "（口令可选，为空则使用 WebDAV 密码）");
                m.put("settings.backup.label.passphrase", "备份口令:");
                m.put("settings.backup.hint.encrypt", "开启后，会先用备份口令加密 API 密钥等敏感信息，再上传到 WebDAV。恢复时需输入相同口令。");
                m.put("settings.backup.button.test", "测试连接");
                m.put("settings.backup.button.config", "配置");
                m.put("settings.backup.button.upload", "立即备份");
                m.put("settings.backup.button.restore", "从远端恢复");
                m.put("settings.backup.status.testing", "正在测试连接...");
                m.put("settings.backup.status.uploading", "正在备份...");
                m.put("settings.backup.status.downloading", "正在读取远端快照...");
                m.put("settings.backup.validation.urlRequired", "请先填写 WebDAV 地址。");
                m.put("settings.backup.validation.encryptKeyRequired", "开启加密但未提供密钥（备份口令或 WebDAV 密码均为空）。");
                m.put("settings.backup.validation.passphraseRequired", "已开启加密，请填写备份口令。");
                m.put("settings.backup.test.success", "连接成功。");
                m.put("settings.backup.test.failed", "连接失败：{0}");
                m.put("settings.backup.upload.success", "备份成功，快照已上传到远端。");
                m.put("settings.backup.upload.failed", "备份失败：{0}");
                m.put("settings.backup.restore.empty", "远端未找到备份快照。");
                m.put("settings.backup.restore.failed", "恢复失败：{0}");
                m.put("settings.backup.restore.decryptFailed", "解密失败，请确认备份口令是否正确。");
                m.put("settings.backup.restore.encryptedButNoKey", "云端快照已加密，但未提供解密密钥。\n请勾选「加密备份」并输入正确的备份口令或 WebDAV 密码。");
                m.put("settings.backup.restore.wrongPassword", "提示：可能是备份口令或 WebDAV 密码错误。");
                m.put("settings.backup.restore.success", "恢复成功，配置已从远端快照写回本地。");
                m.put("settings.backup.restore.confirmTitle", "确认从远端恢复");
                m.put("settings.backup.restore.confirm", "即将用远端快照覆盖本地配置。\n\n快照创建时间：{0}\n来源设备：{1}\n包含：Provider {2} / MCP {3} / Skill {4} / Prompt {5}\n\n此操作会覆盖当前本地配置，是否继续？");
                ...
                m.put("aiSettings.label.gitCommitProfile", "Git 模型:");
                ...
                m.put("aiSettings.label.fimAdapterEnabled", "FIM格式适配:");
                ...
                m.put("provider.dialog.tempLaunch.failed", "临时启动失败: {0}");
                ...
                m.put("providerDialog.label.maxEffort", "最大强度思考");
        ...
        m.put("providerDialog.border.pi", "Pi 配置");
        m.put("providerDialog.label.piProvider", "Provider:");
        m.put("providerDialog.pi.provider.custom", "自定义");
                ...
                m.put("provider.dialog.piAuth.restored",
                                "已恢复该 Pi 官方登录配置的登录状态。");
                m.put("provider.dialog.piAuth.loginRequired",
                                "当前配置尚无已绑定登录状态，下次进入 Pi 需要在终端执行 /login。");
                m.put("provider.dialog.piAuth.snapshotInvalid",
                                "历史登录快照已失效，已清空为未登录状态，下次进入 Pi 需要重新登录。");
        ...
        private static Map<String, String> createEnMessages() {
                ...
                m.put("common.button.refresh", "Refresh");
                m.put("common.button.clearSelection", "Clear Selection");
                ...
                m.put("settings.section.backupSync", "Backup Sync");
                m.put("settings.section.extension", "Extensions");
                m.put("settings.table.cli", "CLI");
                ...
                m.put("settings.option.syncStatus.unsynced", "Unsynced");
                m.put("settings.sync.direction.toCcSwitch", "Export to CC Switch");
                m.put("settings.sync.direction.toCodingSwitch", "Import to Coding Switch");
                m.put("settings.sync.scope.saved", "Saved Config");
                m.put("settings.sync.status.synced", "Synced");
                m.put("settings.sync.status.unsynced", "Unsynced");
                m.put("settings.sync.status.loading", "Loading sync items...");
                ...
                // ── Backup Sync (WebDAV) ──
                m.put("settings.backup.dialog.title", "WebDAV Backup Configuration");
                m.put("settings.backup.hint", "Backup plugin configurations to cloud storage (Jianguoyun, Nextcloud, etc.) via WebDAV for multi-device sync.");
                m.put("settings.backup.label.url", "WebDAV URL:");
                m.put("settings.backup.label.username", "Username:");
                m.put("settings.backup.label.password", "Password:");
                m.put("settings.backup.checkbox.rememberPassword", "Remember WebDAV password");
                m.put("settings.backup.label.remotePath", "Remote snapshot path:");
                m.put("settings.backup.hint.remotePath", "Missing parent directories will be created automatically during upload.");
                m.put("settings.backup.checkbox.autoUpload", "Auto-upload snapshot");
                m.put("settings.backup.label.autoUploadInterval", "Auto-upload interval:");
                m.put("settings.backup.label.interval", "Every");
                m.put("settings.backup.label.intervalUnit", "minutes");
                m.put("settings.backup.hint.autoUpload", "Runs only while IDE is open. Restore downloads still require manual confirmation to avoid accidental overwrites.");
                m.put("settings.backup.hint.plain", "By default, only connection info and settings are synced; sensitive data like API keys are excluded.");
                m.put("settings.backup.checkbox.encrypt", "Encrypt backup");
                m.put("settings.backup.hint.encryptPassphrase", "(Passphrase optional, defaults to WebDAV password)");
                m.put("settings.backup.label.passphrase", "Backup passphrase:");
                m.put("settings.backup.hint.encrypt", "When enabled, sensitive data like API keys will be encrypted with the passphrase before upload. The same passphrase is required for restore.");
                m.put("settings.backup.button.test", "Test Connection");
                m.put("settings.backup.button.config", "Configure");
                m.put("settings.backup.button.upload", "Backup Now");
                m.put("settings.backup.button.restore", "Restore from Remote");
                m.put("settings.backup.status.testing", "Testing connection...");
                m.put("settings.backup.status.uploading", "Backing up...");
                m.put("settings.backup.status.downloading", "Reading remote snapshot...");
                m.put("settings.backup.validation.urlRequired", "Please enter WebDAV URL first.");
                m.put("settings.backup.validation.encryptKeyRequired", "Encryption is enabled but no key provided (both passphrase and WebDAV password are empty).");
                m.put("settings.backup.validation.passphraseRequired", "Encryption is enabled. Please enter backup passphrase.");
                m.put("settings.backup.test.success", "Connection successful.");
                m.put("settings.backup.test.failed", "Connection failed: {0}");
                m.put("settings.backup.upload.success", "Backup successful. Snapshot uploaded to remote.");
                m.put("settings.backup.upload.failed", "Backup failed: {0}");
                m.put("settings.backup.restore.empty", "No backup snapshot found on remote.");
                m.put("settings.backup.restore.failed", "Restore failed: {0}");
                m.put("settings.backup.restore.decryptFailed", "Decryption failed. Please verify the backup passphrase.");
                m.put("settings.backup.restore.encryptedButNoKey", "The cloud snapshot is encrypted, but no decryption key was provided.\nPlease check \"Encrypt backup\" and enter the correct passphrase or WebDAV password.");
                m.put("settings.backup.restore.wrongPassword", "Hint: The backup passphrase or WebDAV password may be incorrect.");
                m.put("settings.backup.restore.success", "Restore successful. Configuration restored from remote snapshot.");
                m.put("settings.backup.restore.confirmTitle", "Confirm Restore from Remote");
                m.put("settings.backup.restore.confirm", "This will overwrite local configuration with the remote snapshot.\n\nSnapshot created: {0}\nSource device: {1}\nContains: Provider {2} / MCP {3} / Skill {4} / Prompt {5}\n\nThis will overwrite your current local configuration. Continue?");
                ...
                m.put("aiSettings.label.gitCommitProfile", "Git model:");
                ...
                m.put("aiSettings.label.fimAdapterEnabled", "FIM adaptation:");
                ...
                m.put("provider.dialog.tempLaunch.failed", "Temporary launch failed: {0}");
                ...
                m.put("providerDialog.label.maxEffort", "Max Effort Thinking");
        ...
        m.put("providerDialog.border.pi", "Pi Config");
        m.put("providerDialog.label.piProvider", "Provider:");
        m.put("providerDialog.pi.provider.custom", "Custom");
                ...
                m.put("provider.dialog.piAuth.restored",
                                "Restored the saved login state for this Pi official login config.");
                m.put("provider.dialog.piAuth.loginRequired",
                                "This config has no saved login state yet. Run /login in Pi the next time it starts.");
                m.put("provider.dialog.piAuth.snapshotInvalid",
                                "The saved login snapshot is no longer valid. It was cleared, and Pi will ask you to sign in next time.");
