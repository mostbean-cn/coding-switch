...
public final class I18n {
        ...
        private static Map<String, String> createZhMessages() {
                ...
                m.put("settings.button.syncCurrent", "单个同步");
                ...
                m.put("settings.hint.extensionSync", "同步 Claude Code、Codex、OpenCode 与 cc-switch 之间的非订阅配置。cc-switch 未安装或未初始化时不会创建数据库，也不会执行同步。");
                ...
                m.put("settings.sync.scope.live", "当前公共配置");
                m.put("settings.sync.liveName", "{0} 当前配置");
                m.put("settings.sync.status.synced", "已同步");
                m.put("settings.sync.status.unsynced", "未同步");
                m.put("settings.sync.status.loading", "正在加载同步项...");
                m.put("settings.sync.status.loaded", "已加载 {0} 个同步项");
                m.put("settings.sync.status.syncing", "正在同步...");
                m.put("settings.sync.current.both", "双方当前");
                m.put("settings.sync.current.source", "仅源端当前");
                m.put("settings.sync.current.target", "仅目标当前");
                m.put("settings.sync.selected.yes", "是");
                m.put("settings.sync.selected.no", "否");
                m.put("settings.sync.details.empty", "请选择左侧同步项查看详情。");
                m.put("settings.sync.details.template",
                                "名称: {0}\nCLI: {1}\n类型: {2}\n方向: {3}\n状态: {4}\n已勾选: {5}\n\n说明: 只同步配置内容，不同步激活状态；官方订阅/官方登录配置会被排除。");
                ...
                m.put("settings.dialog.extensionSync.noCurrentRow", "请先选择一条需要单独同步的配置。");
        ...
        private static Map<String, String> createEnMessages() {
                ...
                m.put("settings.button.syncCurrent", "Sync Current");
                ...
                m.put("settings.hint.extensionSync", "Sync non-subscription configurations between Claude Code, Codex, OpenCode, and cc-switch. No database is created when cc-switch is not installed or initialized.");
                ...
                m.put("settings.sync.scope.live", "Current Common Config");
                m.put("settings.sync.liveName", "{0} Current Config");
                ...
                m.put("settings.sync.status.loaded", "{0} sync item(s) loaded");
                m.put("settings.sync.status.syncing", "Syncing...");
                m.put("settings.sync.current.both", "Current on Both");
                m.put("settings.sync.current.source", "Current on Source");
                m.put("settings.sync.current.target", "Current on Target");
                m.put("settings.sync.selected.yes", "Yes");
                m.put("settings.sync.selected.no", "No");
                m.put("settings.sync.details.empty", "Select a sync item on the left to view details.");
                m.put("settings.sync.details.template",
                                "Name: {0}\nCLI: {1}\nType: {2}\nDirection: {3}\nStatus: {4}\nSelected: {5}\n\nNote: Only configuration content is synced. Active state is not synced. Official subscription/login configurations are excluded.");
                ...
                m.put("settings.dialog.extensionSync.noCurrentRow", "Select one configuration row to sync individually.");
