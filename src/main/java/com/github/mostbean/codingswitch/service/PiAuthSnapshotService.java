...
public final class PiAuthSnapshotService {
    ...
    private static final String SERVICE_NAME = "com.github.mostbean.codingswitch.pi.auth.snapshot";
    private static final String SHARED_NAMESPACE = "pi";
    private static final String SNAPSHOT_USERNAME = "pi-official-login";

    public static PiAuthSnapshotService getInstance() {
        return ApplicationManager.getApplication().getService(PiAuthSnapshotService.class);
    }
    ...
    private void captureFromLive(String primaryKey, String legacyKey) {
        ...
        String rawAuthJson = configFileService.readPiAuthRaw();
        if (PiAuthSupport.isValidOfficialLoginAuth(rawAuthJson)) {
    ...
    private RestoreResult restoreToLive(String primaryKey, String legacyKey) throws IOException {
        ...
        if (snapshotKey != null) {
            ...
            if (PiAuthSupport.isValidOfficialLoginAuth(rawAuthJson)) {
                ...
                configFileService.writePiAuthRaw(rawAuthJson.trim());
                return RestoreResult.RESTORED;
            }
            clearSnapshot(primaryKey, legacyKey);
        }

        String liveAuthJson = configFileService.readPiAuthRaw();
        if (PiAuthSupport.isValidOfficialLoginAuth(liveAuthJson)) {
